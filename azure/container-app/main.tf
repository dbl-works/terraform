...
resource "azurerm_container_app" "main" {
  ...
  template {
    ...
    dynamic "container" {
      ...
      content {
        name    = container.key
        image   = coalesce(container.image, "${var.container_registry_login_server}/${coalesce(var.repository_name, var.project)}:${var.image_version}")
        command = container.value.command
        cpu     = container.value.cpu
        memory  = container.value.memory

        dynamic "env" {
          for_each = container.environment_variables == null ? local.env : { for env, value in container.environment_variables : env => { value = value, secret_name = null } }
          content {
            name        = env.key
