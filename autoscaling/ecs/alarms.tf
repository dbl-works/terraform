locals {
  scale_up_expression   = join(" || ", [for metric in var.autoscale_metrics : "${lower(metric.metric_name)} > ${metric.threshold_up == null ? var.threshold_up : metric.threshold_up}"])
  scale_down_expression = join(" || ", [for metric in var.autoscale_metrics : "${lower(metric.metric_name)} < ${metric.threshold_down == null ? var.threshold_down : metric.threshold_down}"])
}
